...
export const getLogs = async function (): Promise<Output> {
  ...
  const ip =
    headersList.get('x-forwarded-for') || headersList.get('remote-addr') || 'IP not available';
  ...
  try {
    ...
    try {
      ...
      await getSql(
        'INSERT INTO events.logs (type, data, dev, time) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        ['Error', dataString, dev, Date.now()]
      );
      //@ts-ignore
